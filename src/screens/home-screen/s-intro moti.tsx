...
const NiAvatar = () => {
  ...
  let _dark = React.useMemo(() => dark, [dark]);
  function Shape({ image }: { image: string }) {
    return (
      <Motiview
        from={{
          opacity: 0,
  ...
  return (
    <SS.Ctnr>
      <AnimatePresence exitBeforeEnter>
        {_dark && <Shape image={img.khoi} key="khoi" />}
        {!_dark && <Shape image={img.khoi3d} key="khoi3d" />}
      </AnimatePresence>
      <$_FlagRing />
    </SS.Ctnr>
  );
